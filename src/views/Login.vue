...
<template>
 ...
 <div>
    ...
    <p class="mt-2 text-center text-sm text-gray-600">
      Or
      {{ ' ' }}
      <router-link :to="{name: 'Register'}" class="font-medium text-indigo-600 hover:text-indigo-500">Register New Account</router-link>
    </p>
  </div>
  <form class="mt-8 space-y-6" @submit="login" method="POST">
    <div v-if="errorMsg" class="flex items-center justify-between py-3 px-5 my-1  bg-red-500 text-white rounded"> 
      {{ errorMsg }} 
      <span @click="errorMsg = ''" class="w-8 h-8 flex items-center justify-center rounded-full transition-colors cursor-pointer hover:bg-red-700">
    ...
    <div class="-space-y-px rounded-md shadow-sm">
      <div>
        ...
        <div v-if="errorMsg1" class="flex items-center justify-between py-3 px-5 my-1  bg-red-500 text-white rounded"> 
          {{ errorMsg1 }} 
          <span @click="errorMsg1 = ''" class="w-8 h-8 flex items-center justify-center rounded-full transition-colors cursor-pointer hover:bg-[rgba(0,0,0,0.2)]">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </span>
  ...
  function login(ev){
    ...
    store.dispatch('login', user)
      ...
      .catch(err => {
        ...
        errorMsg1.value = err.response.data.message
      });
  }
  </script>